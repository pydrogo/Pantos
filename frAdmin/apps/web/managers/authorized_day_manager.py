...
class AuthorizedDayManager(Manager):
    def search(self,groupid=-1, source_id=0, source_type=1, per_page=10, page=0):
        query_search = self.get_queryset().filter(group_id=groupid).all()
        ...
        return list(query_search.values('group','date','id','start','end')), count_query
